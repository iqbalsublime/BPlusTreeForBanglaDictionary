# -*- coding: utf-8 -*-
"""
Created on Sun Oct 13 20:05:14 2019
@author: iqbalsublime
"""
...
def initBanglaDictionary2():
    bplustree = BPlusTree(order=7)
    print('\nB+ tree with 1 item...')
    bplustree.insert('Apply', 'আবেদন')
    bplustree.show()

    print('\nB+ tree with 2 items...')
    bplustree.insert('Ball', 'বল')
    bplustree.show()

    print('\nB+ tree with 3 items...')
    bplustree.insert('Cat', 'বিড়াল')
    bplustree.show()

    print('\nB+ tree with 4 items...')
    bplustree.insert('Dog', 'কুকুর')
    bplustree.show()

    print('\nB+ tree with 5 items...')
    bplustree.insert('Eagle', 'ঈগল')
    bplustree.show()

    print('\nB+ tree with 6 items...')
    bplustree.insert('Fan', 'পাখা') 
    bplustree.show()

    print('\nRetrieving values with key Fan...')
    print(bplustree.retrieve('Fan'))

def initBanglaDictionary(keys, values): 
    bplustree = BPlusTree(order=7)
    index=0
    for key in keys:
         print('\nB+ tree with KEYS...', index+1)
         bplustree.insert(keys[index], values[index])
         #bplustree.show()
         index=index+1
         
    print('\nRetrieving values with key mwanawasa...')
    print(bplustree.retrieve('mwanawasa'))  
    print('\nRetrieving values with key theories...')
    print(bplustree.retrieve('theories'))  
    #bplustree.show()
        

if __name__ == '__main__':
    #demo_node()
    f = open("bptree.txt", 'r', encoding="utf-8")
    ...
    #print(keys)
    #print(values)        
    initBanglaDictionary(keys, values)
